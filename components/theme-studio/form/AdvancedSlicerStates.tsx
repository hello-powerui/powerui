...
interface AdvancedSlicerStatesProps {
  selectedState: string;
  onSelectedStateChange: (state: string) => void;
  visualType: string;
}

// Define the available states for advanced slicer
const ADVANCED_SLICER_STATES_COMPLEX = {
  basic: [
...
export function AdvancedSlicerStates({ 
  selectedState, 
  onSelectedStateChange,
  visualType
}: AdvancedSlicerStatesProps) {
  // Check if this is an advanced slicer visual
  const isAdvancedSlicer = visualType === 'advancedSlicerVisual';
  ...
  // For advanced slicer visual, show advanced state selector
  return (
    <Card className="mb-3 p-4 border-gray-200 shadow-sm">
      <div className="space-y-3">
        <div>
        ...
        <Select value={selectedState} onValueChange={onSelectedStateChange}>
          <SelectTrigger className="w-full">
            <SelectValue placeholder="Select a state">
              <div className="flex items-center gap-2">
                <span>{ADVANCED_SLICER_STATES_COMPLEX.basic.find(s => s.value === selectedState)?.label || 
                       ADVANCED_SLICER_STATES_COMPLEX.expansion.find(s => s.value === selectedState)?.label ||
                       ADVANCED_SLICER_STATES_COMPLEX.series.find(s => s.value === selectedState)?.label ||
                       ADVANCED_SLICER_STATES_COMPLEX.combinations.find(s => s.value === selectedState)?.label ||
                       selectedState}</span>
          ...
          <SelectContent>
            <div className="p-1">
              ...
              {ADVANCED_SLICER_STATES_COMPLEX.basic.map((state) => (
              ...
              {ADVANCED_SLICER_STATES_COMPLEX.expansion.map((state) => (
                <SelectItem key={state.value} value={state.value}>
                  <div className="flex flex-col">
                    <span className="font-medium">{state.label}</span>
                    <span className="text-xs text-gray-500">{state.description}</span>
              ...
              {ADVANCED_SLICER_STATES_COMPLEX.series.map((state) => (
                <SelectItem key={state.value} value={state.value}>
                  <div className="flex flex-col">
                    <span className="font-medium">{state.label}</span>
                    <span className="text-xs text-gray-500">{state.description}</span>
              ...
              {ADVANCED_SLICER_STATES_COMPLEX.combinations.map((state) => (
                <SelectItem key={state.value} value={state.value}>
                  <div className="flex flex-col">
                    <span className="font-medium">{state.label}</span>
                    <span className="text-xs text-gray-500">{state.description}</span>
