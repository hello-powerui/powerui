...
export function StateAwarePropertySection({
  ...
  // Handle clearing the current state
  const handleClearState = () => {
    // Remove the current state from the array
    const newArray = arrayValue.filter(item => item.$id !== globalSelectedState);
    
    // For fill/text properties, preserve the show object
    if (isFillProperty || isFillCustomProperty || isTextProperty) {
      const showObject = arrayValue.find(item => !item.$id && 'show' in item);
      if (showObject) {
        newArray.push(showObject);
      }
    }
  ...
  const currentStateHasContent = stateItem && Object.keys(stateItem).length > 1; // More than just $id
    ...
    <div className="space-y-2">
      {!hideTitle && schema.title && (
        <div className="flex items-center justify-between">
          ...
          {currentStateHasContent && (
            <button
              onClick={handleClearState}
              className="text-xs text-gray-500 hover:text-red-600 flex items-center gap-1"
              title={`Clear ${globalSelectedState} state`}
            >
              <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
              </svg>
              Clear {globalSelectedState}
            </button>
          )}
        </div>
      )}
