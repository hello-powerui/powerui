...
export default function SimplePowerBIEmbed({ 
  ...
  // Load report only when theme is ready
  useEffect(() => {
    console.log('[PowerBIEmbed] Theme check:', { 
      hasTheme: !!variantPreviewTheme, 
      hasEmbedConfig: !!embedConfig, 
      isLoading,
      theme: variantPreviewTheme 
    });
    
    // Skip if no theme or already loading/loaded
    if (!variantPreviewTheme || embedConfig || isLoading) {
      return;
    }
    ...
    const loadReport = async () => {
      try {
        ...
        const tokenType = config.tokenType === 1 ? models.TokenType.Embed : models.TokenType.Aad;
        
        console.log('[PowerBIEmbed] Using token type:', tokenType === models.TokenType.Embed ? 'Embed' : 'AAD');
        ...
        console.log('[PowerBIEmbed] Setting embed config:', embedConfiguration);
    ...
    loadReport();
  }, [variantPreviewTheme]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const eventHandlers = new Map([
    ['loaded', function () {
      console.log('[PowerBIEmbed] Report loaded');
      setIsReportReady(true);
      // Discover visuals immediately after report loads
      discoverVisuals();
    }],
    ['rendered', function() {
      console.log('[PowerBIEmbed] Report rendered');
      // Ensure report is marked as ready on render too
      setIsReportReady(true);
    }],
    ['error', function (event: any) {
      console.error('[PowerBIEmbed] Error:', event);
      console.error('[PowerBIEmbed] Error details:', event?.detail);
      if (event?.detail?.message) {
        setError(event.detail.message);
      } else if (event?.detail) {
        setError(JSON.stringify(event.detail));
      }
    }],
    ['pageChanged', function () {
      console.log('[PowerBIEmbed] Page changed');
    ...
    <div className="w-full h-full relative">
      ...
      <PowerBIEmbed
        ...
        getEmbeddedComponent={(embeddedReport) => {
          console.log('[PowerBIEmbed] Embedded component received:', embeddedReport);
